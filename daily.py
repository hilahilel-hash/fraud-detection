...
def explain_transaction_scores(df, model, features, weights, thresholds):
    ...
    # v13.1: fixed cutoffs restored — the ML-score normalization put final_score
    # back on a meaningful [0,1] scale (top fraud ~0.99), so absolute tiers work
    # again and are clearer than percentiles ("0.8 = critical").
    def explain_cat(row):
        s = row["final_score"]
...
def generate_summary_report(df):
    ...
    print("=" * 80)
    # v13.1: fixed tiers (ML normalization restored a meaningful [0,1] scale).
    print(f"Mean final score:         {df['final_score'].mean():.4f}")
    print(f"CRITICAL (>=0.8):         {(df['final_score'] >= 0.8).sum()}")
    print(f"HIGH (0.6-0.8):           {((df['final_score'] >= 0.6) & (df['final_score'] < 0.8)).sum()}")
    print(f"MEDIUM (0.4-0.6):         {((df['final_score'] >= 0.4) & (df['final_score'] < 0.6)).sum()}")
